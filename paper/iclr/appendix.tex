...
\Cref{tab:claims-evidence} lists every claim the paper makes, what was
measured, the retained file that carries the number, and what the claim does
not license. Rows are grouped as the paper is: the primary live families, the
external substrates, and the studies added after the pre-submission review.

\begin{table}[p]
  \centering
  \caption{Claims, evidence, retained sources, and claim boundaries. Every
  number is copied from the file named in the third column; the number
  registry in the artifact maps each to its producing script.}
