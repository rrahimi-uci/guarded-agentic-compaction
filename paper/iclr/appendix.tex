...
$m=16$, crossing $\delta=0.10$ past the eleven-point grid), while the
...
with no witness is treated as a genuine model decision and a slot with more than
$\kappa$ witnesses is treated as ambiguous, so the analysis never resolves an
argument by picking the cheapest available explanation.
...
server-side response storage disabled. Sampling parameters are not overridden
and take provider defaults. Reported dollars are an \emph{estimate}, computed by
...
requires support across independent scenario groups.
...
to certify a ranking --- and these experiments do not separate them. A frontier
study must first buy calibration groups. The implementation also supports a
...
Per-record paired differences (compiled minus baseline) on the two families that
admit a deep prefix, with 10{,}000-sample bootstrap intervals and two-sided
Wilcoxon signed-rank tests over $n=30$ paired held-out records:
...
evidence for an average effect: all runs share the same provider family and the
same narrow two-read outcome task.
...
not as a demonstrated risk--coverage frontier. The exploratory partial frontiers
therefore remain diagnostic rather than evidence for the main claim.
...
repositories admit an artifact and complete 240 held-out triples under all three
arms; \code{pytorch/pytorch} again retires at compile time --- the same
repository, under an independent five-times-larger cohort, reproducing rather
than revealing an artifact of the smaller core cohort's retirement. The pooled
result is therefore 240 of the pre-registered 300 pairs, short of target for the
stated reason, not concealed.
...
repositories never accept a calibration group below $\eta=0.11$ and accept
all 92 at $\eta\ge.11$, a pure two-point step; the fourth,
\code{huggingface/datasets}, additionally accepts 10 of 92 groups at
$\eta\le 0.08$, but that point's exact upper bound (0.375) exceeds the
registered $\alpha=0.05$ budget and is never admissible, so every repository
...
fourth listed outcome: neither gate produces graded coverage on this cohort, so
the null is reported rather than a manufactured frontier. At four times the
prior cross-repository scale, and against a comparator that shares every
statistical mechanism except the risk budget, the exact-$\alpha=0.05$ gate
remains a support threshold. This confirms, and does not resolve, the step-gate
finding \cref{sec:discussion} already reports.
