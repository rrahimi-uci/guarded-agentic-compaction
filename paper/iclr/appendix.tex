...
(\code{paper/supplementary/multiplicity-repair-protocol.md}), failed its own
...
\begin{figure}[t]
  ...
  \caption{Why a 92-group pool certifies only full coverage. Left: the
  zero-violation bound $U(n)=1-(\delta/|\Lambda|)^{1/n}$ at $\delta=0.1$,
  $|\Lambda|=11$ crosses $\alpha=0.05$ between $n=91$ (0.0503) and $n=92$
  (0.0498). Right: the smallest pool $\lceil 92/c\rceil$ that can certify
  selective coverage $c$; every calibration pool in this paper is 92, so the
...
extension (\code{paper/supplementary/issue-type-extended-heldout-protocol.md})
...
the same-model baseline, matching the primary family within a point.
...
\begin{table}[t]
  ...
  \footnotesize
  \input{tables/extended_heldout.tex}
\end{table}
...
evidence composition rather than on trace structure. A provider-free
recompilation from the retained discovery checkpoint
(\code{paper/results/iclr\_revision/pytorch\_core\_recompile.json}) reproduces
the retirement and recovers the per-threshold rows the original run truncated:
$n_\eta=0$ at every $\eta\in\Lambda$, so no calibration group was accepted at
any threshold and the bound is the degenerate branch of \cref{eq:cp}. The rows
identify the branch, not its cause; the class-composition explanation is
consistent with the balanced rerun's admission but is not established by them.
...
(\code{paper/supplementary/read-only-prologue-protocol.md}): an artifact may
...
documentation call as the prologue (\cref{tab:appworld-prologue}), it changes
nothing: ReAct eligibility rises from 0 to 2 of 2{,}340 and plan-and-execute
...
architectures were refused; the underlying one is that they do not execute the
admitted region at all, so no prologue rule can dispatch it. The prologue tool
...
\begin{table}[t]
  ...
  \scriptsize
  \setlength{\tabcolsep}{3pt}
  \input{tables/appworld_dispatch_prologue.tex}
\end{table}
...
to compare our gate against, only a cache-hit rate. \Citet{wei2026evoc2f}
compiles plans into an annotated intermediate representation and admits learned
skills to a library only after verification tests, and \citet{winston2026agentjit}
compiles a task description into code validated against tool pre- and
postconditions and schedules it for latency; in both, the model still plans
each task, and admission is a test or specification check within an episode,
not a bound on post-substitution risk over a calibration distribution, so
neither supplies the admissibility decision our gate would be compared
against. Re-expressing any of them as
...
(\code{paper/supplementary/recurrence-only-ablation-protocol.md}). A
...
\begin{table}[t]
  ...
  the last replays the three reads the compiler refused (\code{limit=100} has
  no witness) with every barrier disabled and one provider request. All four
  ...
  \footnotesize
  \input{tables/recurrence_only.tex}
\end{table}
...
(\code{paper/supplementary/second-model-replication-protocol.md}) reran the
...
(design A, \code{paper/supplementary/second-model-rediscovery-protocol.md})
...
below the 116 the 16/8/92 split needs, because the model's own excerpts were
ungrounded on 19 records; no held-out arm was run and no artifact is claimed.
...
(\code{paper/supplementary/second-provider-replication-protocol.md}): adaptive
...
reached 126, 128, and 116 exact traces of 132. Issue-type and PR-outcome
...
at 0.050 and 0.051, above $\alpha$: the floor of \cref{fig:gate-floor} binding
...
counts as a failure) and 27/27/29 on PR-outcome; requests fell 48.3\% and
...
\begin{table}[t]
  ...
  \footnotesize
  \setlength{\tabcolsep}{4pt}
  \input{tables/second_provider.tex}
\end{table}
...
all-or-none support tests (\cref{fig:gate-support}), and the candidate-search
problem is compiler-wide only when one candidate reaches calibration
(\cref{cor:frozen}) or the budget is divided across those that do; for the two
families whose dominance search calibrated two candidates the corrected bound is
0.0569 (\cref{tab:multiplicity-accounting}). Likewise, exact task
contracts do not certify full semantic equivalence of the final answer,
especially once a downstream model continuation is involved. Two operational
...
discovery checkpoints (\code{paper/scripts/recompile\_with\_challenge.py})
...
(\code{paper/results/iclr\_revision/recompile\_with\_challenge.json}); no
...
adverse finding reported in full, ahead of every other reading. Neither gate
producing graded coverage is the fourth pre-declared outcome: the workload
supplied no gradient, and no frontier is to be manufactured from a homogeneous
cohort. Two extensions
...
\Cref{tab:claims-evidence,tab:claims-evidence-b,tab:claims-evidence-c,tab:claims-evidence-d} list every claim the paper
makes, what was measured, the retained file that carries the number, and what
the claim does not license. Rows are grouped as the paper is: the primary live families, the
external substrates, and the studies added after the pre-submission review.

\begin{table}[p]
  \centering
  \caption{Claims, evidence, retained sources, and claim boundaries, part A: the primary families. Every number is copied from the file named in the third column; the number registry in the artifact maps each to its producing script.}
  \label{tab:claims-evidence}
  \scriptsize
  \setlength{\tabcolsep}{3pt}
  \renewcommand{\arraystretch}{1.05}
  \input{tables/claims_evidence_a.tex}
\end{table}

\begin{table}[p]
  \centering
  \caption{Claims and evidence, part B: the cross-repository studies, the external substrates, and the recurrence-only replay.}
  \label{tab:claims-evidence-b}
  \scriptsize
  \setlength{\tabcolsep}{3pt}
  \renewcommand{\arraystretch}{1.05}
  \input{tables/claims_evidence_b.tex}
\end{table}

\begin{table}[p]
  \centering
  \caption{Claims and evidence, part C: the second model family, the second provider, the multiplicity-repair NO-GO, and group independence.}
  \label{tab:claims-evidence-c}
  \scriptsize
  \setlength{\tabcolsep}{3pt}
  \renewcommand{\arraystretch}{1.05}
  \input{tables/claims_evidence_c.tex}
\end{table}

\begin{table}[p]
  \centering
  \caption{Claims and evidence, part D: the continuation check, the extended held-out cohort, the read-only prologue, and the closed operational caveats.}
  \label{tab:claims-evidence-d}
  \scriptsize
  \setlength{\tabcolsep}{3pt}
  \renewcommand{\arraystretch}{1.05}
  \input{tables/claims_evidence_d.tex}
\end{table}
