...
\ForAll{partitions $\mathcal{E}_M$}
  ...
  \State $(G,S)\gets\textsc{BuildPatg}(\mathcal{T},\Theta,\mathcal{C},\mathcal{L},\kappa)$ \Comment{\cref{alg:patg}: annotate every slot}
  \State $\mathcal{F}\gets\textsc{Canonicalize}(\textsc{MineRegions}
         (G,S,\mathcal{C},w_{\max}))$, keeping only cross-group support
         \Comment{rejects any window with an \textsc{ungrounded} or \textsc{ambiguous} slot}
  \If{$\textsc{Ceiling}(\mathcal{T},\mathcal{F}) < \Delta$} \hfill\textsc{(caller)}
     \State \textbf{record} \retire{} \textbf{(infeasible ceiling, Eq.~\eqref{eq:ceiling})};
            \textbf{continue}
