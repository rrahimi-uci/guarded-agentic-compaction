...
Consider issue \#6602 (``Index error when data is large'') from the public
GitHub snapshot. Given \code{issue\_number=6602}, the agent repeatedly reads
\code{record}, \code{labels}, and \code{comments(limit=3)} before rendering an
auditable triage answer. A recurrence-only optimizer would emit that whole
three-read macro. GAC instead produces two decisions: it \emph{retires} the
full candidate because the integer \code{comments.limit} varies across traces
and has no consistent provenance expression, then it emits the grounded
\code{record -> labels} prefix and returns control to the agent for comments
and rendering. The emitted prefix passes 92/92 calibration groups at
$\alpha=.05$ (upper bound 0.0498).

This partial artifact is not a cosmetic compromise. In an earlier 18-record
follow-up, the aggressive three-read artifact replayed all 45 tool calls but
produced only 17/18 exact downstream answers: for this issue, it dropped a URL
from a Markdown link even though its tool contract passed. A provider-free
continuation guard detects and checked-renders that miss; it is a mechanism
check, not a live safety claim. Recurrence says what may be compilable;
provenance, continuation scope, and evidence determine what may replace the
model.
