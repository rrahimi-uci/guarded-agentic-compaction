...
These are likely invariants~\citep{ernst2001daikon}, not proofs for all future
inputs --- which is why a statistical gate follows them rather than replacing
them.
...
\begin{equation}
  ...
  \qquad
  U_\eta \Big|_{k_\eta=0} = 1-\left(\tfrac{\delta}{|\Lambda|}\right)^{1/n_\eta}.
  \label{eq:cp}
\end{equation}
...
discipline~\citep{angelopoulos2022ltt,bates2021rcps}. The closed form on the
...
Fix one candidate program, hard guard, verifier, score, and finite threshold
grid $\Lambda$ before calibration. If admitted calibration groups are i.i.d.,
then with probability at least $1-\delta$ every $\eta\in\Lambda$ satisfies
$r_\eta\le U_\eta$, where $r_\eta$ is the selective risk at $\eta$ and $U_\eta$
is given by \cref{eq:cp}. Hence the data-selected threshold satisfies the
target selective risk of \cref{eq:objective} whenever its upper bound is at
most $\alpha$.
\end{proposition}
