...
\begin{table}[t]
  ...
  a recovered producer and $n_{\max}$ the largest family support against the 92
  \cref{eq:cp} requires.}
  \label{tab:funnel}
  \footnotesize
...
(\cref{tab:appworld-dispatch}). A miss abstains cleanly, and this bounds $\phi$
from above rather than measuring it, so no saving is claimed.
