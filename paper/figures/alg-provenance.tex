% Algorithm 2: typed argument provenance (PATG).
\begin{algorithm}[t]
...
\small
\begin{algorithmic}[1]
\Require episodes $\mathcal{T}$; entry schema $\Theta$; library $\mathcal{L}$;
         ambiguity cap $\kappa$
\Ensure provenance graph $G$ with an edge per grounded argument slot
  ...
  \ForAll{tool calls $c_j\in e_{1:T}$ in order}
    \ForAll{argument slots $u\in\operatorname{args}(c_j)$}
      ...
      \If{$u$ is declared \emph{literal-only} for $c_j$}
        \State $\mathcal{H}\gets\{\sigma\in\mathcal{H} :
               g=\mathrm{id}\ \text{or}\ g\ \text{constant}\}$
      ...
      \EndIf
    \EndFor
    \State $\Sigma\gets\Sigma\cup\{(\text{var}(c_j),\pi,v) :
           (\pi,v)\in\textsc{Flatten}(o_j)\}$
           \Comment{the result becomes a witness for later slots only}
  \EndFor
  \ForAll{pairs $(e_s,e_t)$ sharing a resource where either writes it}
    \State $G\gets G\cup\{e_s\prec e_t\}$
           \Comment{order edge: a reordering can never be proposed across a conflict}
  \EndFor
\EndFor
\State \Return $G$
\end{algorithmic}
