% Algorithm 4: runtime dispatch with staged execution and baseline fallback.
\begin{algorithm}[t]
...
\small
\begin{algorithmic}[1]
\Require entry state $z$; runtime context $M'$; registry $\mathcal{R}$; mode
...
\EndIf
\If{$m=\textsf{live}\ \land\ \exists f\in\operatorname{tools}(A.P):
     \mathcal{C}(f).\textit{quota\_attested}\ \land\ \textsc{Snapshot}=\bot$}
  \State \Return $\fallback$
...
\EndIf
\State $S\gets\textsc{Stage}.\textsc{Begin}()$
\State $r\gets\textsc{Interpret}(A.P,z,\textsc{Facade}(\mathcal{C}))$
       \Comment{bounded program; the facade re-checks effects at execution time}
...
\EndIf
\State $S.\textsc{Commit}(r.\text{effects})$; \Return $r.\text{observations}$
\end{algorithmic}
\end{algorithm}
