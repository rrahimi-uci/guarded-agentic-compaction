...
C22 & Every reported artifact meets the registered 5\% selective-risk target & results/admission\_register.json: four artifacts at alpha=.05, three at alpha=.10 & Contradicted; the GCS and earlier three-read artifacts are licensed only at 10\% and would retire at 5\% \\
C23 & Token reduction is a proxy for cost reduction & results/cache\_accounting.json: macro uses 30.9\% fewer tokens but is 8.0\% cheaper at 0.0\% cache reads vs. 27.8\% & Contradicted; interface fusion trades prompt length against prefix reuse and the two newer families are cache-cold throughout \\
