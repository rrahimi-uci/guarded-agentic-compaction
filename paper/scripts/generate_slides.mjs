...
async function main() {
  ...
  const manifest = {
    ...
    evidence_boundary: "Three real-record workflow families support the primary transfer result: compiled 90/90, baseline 89/90, manual 90/90, all admitted at the registered alpha=.05 with a pooled 3.3% compiled-only discordance bound. The Headroom ablation is limited to two reused 30-record cohorts: its 240 eligible JSON payloads received zero transformations and saved zero tokens, so it is a boundary-specific negative result rather than a rival mechanism for GAC savings. The snapshot does not establish cross-repository or time-forward transfer, and part of the reported 32.0-75.3% cost range reflects prompt-cache warmth rather than compiled depth. GCS and comparator results rest on an artifact calibrated at alpha=.10 and are not licensed at .05. NESTFUL and API-Bank remain refusal evidence; eight other benchmark paths are supplementary interoperability audits. A prospective gate-frontier study (four of five sealed repositories, 240 of the pre-registered 300 held-out pairs; the fifth retires at compile time, reproducing the cross-repository extension's own finding) ran a support-only comparator -- alpha=1, otherwise a byte-for-byte identical compile pass -- against the learned gate. Both are statistically indistinguishable on every metric, and every repository deploys the same coverage-1.0 threshold: the pre-declared null, not a demonstrated frontier.",
  };
