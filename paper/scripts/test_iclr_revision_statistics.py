...
COMMITTED_TABLES = stats.TABLES
COMMITTED_JSON = stats.OUT


@pytest.fixture(scope="module", autouse=True)
def _scratch_outputs(tmp_path_factory):
    """Regenerate into a scratch directory.

    The committed JSON and tables are inputs to the publication-manifest check;
    a test must never rewrite them (float repr differs across Python versions).
    """
    scratch = tmp_path_factory.mktemp("iclr_revision")
    stats.OUT = scratch / "json"
    stats.TABLES = scratch / "tables"
    yield
    stats.OUT = COMMITTED_JSON
    stats.TABLES = COMMITTED_TABLES
...
def test_generated_tables_exist_and_are_marked() -> None:
    for name in ("absolute_resources", "paired_statistics", "discordance", "multiplicity_accounting",
        ...
        text = (COMMITTED_TABLES / f"{name}.tex").read_text()
