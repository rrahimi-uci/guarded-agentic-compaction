...
| A simplified cross-repository, time-forward GitHub task supports guarded compilation | `results/github_multirepo_pr_outcome_core/results.json` | Verified on sample | five repositories sealed; 580/580 exact discovery traces; four repositories complete 120/120 held-out exact pairs; compiled requests -44.4%, total tokens -52.4%, observed latency -49.4%, estimated cost -48.6%; one repository retires at compile time |
...
| The three-family result alone generalizes across repositories or time | pinned source revision and selections | Not established | all 90 held-out records come from one repository snapshot; cohorts are workflow-distinct, not cross-repository or time-forward |
| The cross-repository extension proves broad workflow-general superiority | `results/github_multirepo_pr_outcome_core/results.json` | Not supported | the extension is a narrower two-read PR-outcome task; a fixed template is more efficient there, and the learned artifact falls back on all open pull requests |
...
| The implemented portfolio can recommend a measured macro | `src/guarded_agentic_compaction/portfolio/`, `results/portfolio_live/results.json` | Verified on one family | both measured actions admitted on 30 groups; macro utility 0.489 vs compiler 0.327; recommendation requires human review |
...
| Per-candidate exact gates provide compiler-wide control after candidate-family search | `src/guarded_agentic_compaction/grc/compile.py`, `src/guarded_agentic_compaction/grc/calibrate.py`, `results/admission_register.json` | Not established | delta is split over 11 thresholds for one fixed candidate, while the compiler may calibrate several families; two-candidate Bonferroni control would require 106 zero-violation groups rather than 92 |
