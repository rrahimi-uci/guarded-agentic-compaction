...
| [Headroom](https://github.com/headroomlabs-ai/headroom) and context-compression middleware | compresses tool outputs, logs and retrieved chunks in flight; self-reports 60--95% token reduction on JSON and 73% on a GitHub triage workload | shrinks the payload of calls that still happen and never removes a model boundary, so it cannot reduce provider requests at all; admits on measured benchmark deltas over lossy compression, with no abstention path and no finite-sample bound | `reference` — the orthogonal axis, and the comparator this repository's token numbers most need: every reported reduction here is against an *uncompressed* baseline, and the composition of both was not run |
