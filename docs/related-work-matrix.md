...
| Hand-written composite tool (+ concurrent reads) | collapses a read prefix into one call a person can read | a provider-visible macro still spends a request selecting it; a manually pre-executed macro can remove that turn but is not measured | `comparator` — scored in the offline and natural GitHub studies; GCS later beats the measured provider-visible macro on 12 exploratory pairs while preserving three internal source reads |
